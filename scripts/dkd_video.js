...
//let HeaderArr = [], dkdvdHeader = "";
  ...
  else if (process.env.DKD_VIDEO_BD && process.env.DKD_VIDEO_BD.indexOf('\n') > -1) {
  dkdvdBody = process.env.DKD_VIDEO_BD.split('\n');
  console.log(`您选择的是用换行隔开\n`)
  } else {
  dkdvdBody = process.env.DKD_VIDEO_BD.split()
  }
  Object.keys(dkdvdBody).forEach((item) => {
        if (dkdvdBody[item]) {
 ...
 /*   
  ...
  else if (process.env.DKD_VIDEO_HD && process.env.DKD_VIDEO_HD.indexOf('\n') > -1) {
  dkdvdHeader = process.env.DKD_VIDEO_HD.split('\n');
  console.log(`您选择的是用换行隔开\n`)
  } else {
  dkdvdHeader = process.env.DKD_VIDEO_HD.split()
  }
  Object.keys(dkdvdHeader).forEach((item) => {
        if (dkdvdHeader[item]) {
